...
class SVMWrapper:
    ...
    def train(self, images, labels):
        ...
        selected_labels = []
        for train_label in tqdm(train_labels, desc='run all frames '):
            selected_labels.append(float(train_label))
        ### we need to try out the svm model
        print('SVM training start....')
        self.clf.fit(train_images, selected_labels)
    ...
    def initialize_train_dataset(self, images, labels):
        ...
        random_idx = np.random.choice(rand_arr, int(len(images)*0.1))
