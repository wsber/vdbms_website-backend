...
class EKO_mab(EKO_alternate):
    ...
    # 这个函数进行批量MABS采样
    def build_additional_anchors_(self, target_dnn_cache, scoring_func):
        """
        ...
        for i in tqdm(range(int((n_reps - curr_len) / selbath)),
                      desc='MABSampling: Here exe select N-alpha*N times MABSampling'):
            ...
            cluster_keys = []
            new_reps = []
            new_reps, cluster_keys = self.select_rep_batch(cluster_dict, rep_indices, selbath)
            rep_indices.extend(new_reps)
            # print('[len_reps]: ',len(new_reps))
            cluster_dict = self.update_batch(cluster_dict, cluster_keys, new_reps, target_dnn_cache, scoring_func)
    ...
    # 批处理采样后更新每个臂的奖励
    def update_batch(self, cluster_dict, cluster_keys, rep_idxs, target_dnn_cache, scoring_func):
        ### I mean we can just update everything here.....
        oracle_results = self.run_oracle_model_batch(rep_idxs)
        for rep_idx, oracle_result, cluster_key in zip(rep_idxs, oracle_results, cluster_keys):
            ...
            cluster_dict[(start_idx, end_idx)]['cache'].append(scoring_func(oracle_result))
            cluster_dict[(start_idx, end_idx)]['distance'] = np.mean(cluster_dict[(start_idx, end_idx)]['cache'])
        return cluster_dict

    def select_rep_batch(self, cluster_dict, rep_indices, num_segments=16):
        """
        从聚类字典中选择多个最优片段，并从每个片段中随机抽取一帧作为代表帧。
        Args:
            cluster_dict (dict): 包含聚类信息的字典
            rep_indices (list): 代表帧的索引列表
            num_segments (int): 选择的最优片段数量

        Returns:
            list: 包含选定片段的信息的列表，每个元素为 (start_idx, end_idx, rep_idx) 元组
        """
        rep_idxs = []
        cluster_keys = []
        # 计算每个聚类的 MAB 值
        ...
        # 选择具有最高 MAB 值的前 num_segments 个聚类
        selected_clusters = np.argsort(mab_values)[-num_segments:]  # 获取前 num_segments 个最优聚类的索引
        segment_choices = []
        for cluster_idx in selected_clusters:
            start_idx, end_idx = list(cluster_dict.keys())[cluster_idx]
            # 找出选定片段中可用的帧
            choices = [i for i in range(start_idx, end_idx + 1) if
                       i not in cluster_dict[(start_idx, end_idx)]['members']]
            if len(choices) == 0:
                # 如果在此片段中找不到可用帧，则将此片段的距离奖励设置为 0，并递归选择新片段
                cluster_dict[(start_idx, end_idx)]['distance'] = 0
                rep_idx, (start_idx, end_idx) = self.select_rep(cluster_dict, rep_indices)
            else:
                # 从可用帧中随机选择一个作为代表帧
                rep_idx = np.random.choice(choices)

            # 将选择的片段信息添加到列表中
            rep_idxs.append(rep_idx)
            cluster_keys.append((start_idx, end_idx))

        return rep_idxs, cluster_keys

    def run_oracle_model_batch(self, reps):
        oracle_results = []
        images = np.array([self.images[i] for i in reps])
        iter = 0
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                for prediction in y:
                    ...
                    oracle_results.append(self.index_cache[rep])
    ...
    def build_additional_anchors(self, target_dnn_cache, scoring_func):
        ...
        for i in tqdm(range((n_reps - curr_len)), desc='MABSampling: Here exe select N-alpha*N times MABSampling'):
            ...
            # new_rep, cluster_key = self.select_rep_alpha(cluster_dict, rep_indices)
